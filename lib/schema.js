...
export function legalServiceSchema(t) {
  return {
    ...
    email: site.email,
    image: `${siteUrl}/images/esther-pujol-wilkie-abogada-sotogrande.jpg`,
    priceRange: '$$',
    address: {
      '@type': 'PostalAddress',
    ...
    foundingDate: String(site.admittedYear),
    member: {
      '@type': 'Person',
      name: site.colegiadaName,
      alternateName: 'Esther Pujol Wilkie',
      jobTitle: 'Abogada',
      hasCredential: {
        '@type': 'EducationalOccupationalCredential',
        credentialCategory: 'Professional licence',
        name: `Abogada ejerciente, ${site.colegio}, nº ${site.colegiadoNumber}`,
        recognizedBy: { '@type': 'Organization', name: site.colegio, url: site.colegioUrl },
        dateCreated: site.admittedDate,
      },
      ...(site.cpd && site.cpd.length
        ? { hasOccupation: { '@type': 'Occupation', name: 'Abogada' },
            award: site.cpd.map((c) => `${c.name}, ${c.issuer}, ${c.hours}h, ${c.completedLabel}`) }
        : {}),
    },
    areaServed: ['Sotogrande', 'San Roque', 'Estepona', 'Marbella', 'Gibraltar', 'Costa del Sol'],
    knowsAbout: t.services.items.map((s) => s.title),
    sameAs: [site.mapsUrl, 'https://www.linkedin.com/in/estherpujol/'],
  };
}
...
export function articleSchema(post, { locale, path }) {
  ...
  return {
    ...
    image: `${siteUrl}/images/sotogrande-marina-cadiz.jpg`,
  };
}
